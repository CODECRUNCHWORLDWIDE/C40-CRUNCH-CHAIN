...
**C** — `0.15 × 400,000 = 60,000`; `0.05 × 1,200,000 = 60,000` — same expected annual loss despite very different likelihood/impact shapes. (Check the others: A = 360,000; B = 58,500 ≈ close but not exact; D = 6,000 — C is the cleanest match.)
...
**C** — the defining test is "if this went to zero tomorrow, is there a Plan B, or would we be improvising?"
...
**B** — likelihood 4 × impact 5 = 20, and it's flagged `single_point_of_failure = TRUE`, all three factors driving the top score.
...
**B** — rare-but-severe risks are the classic insurance/transfer case; engineering around every low-likelihood risk is usually not cost-effective.
...
**D** — visibility shrinks detection time, not the disruption's underlying severity; buffers/redundancy/flexibility address severity directly.
...
**B** — averaging a bad week into three normal weeks produces a monthly figure that looks only mildly off, hiding the real severity of the bad days.
...
**B** — including today's anomalous value in its own baseline average would pull the baseline down too, shrinking the very gap the query is trying to measure.
...
**A** — a materialized view is a stored, schedule-refreshed result; a plain view recomputes its underlying query every time it's referenced.
...
**B** — z = -2.4 is a meaningful deviation (beyond the ~95% two-sigma band) but not yet at the ~99.7% three-sigma extreme.
...
**B** — IQR is typically computed once over an entire historical series; a rolling z-score recomputes its baseline continuously as new data arrives.
...
**B** — isolated noisy days rarely repeat two days running, while a genuine disruption's effects compound and persist, which is exactly what the persistence rule is built to distinguish.
...
**B** — narrow, low-cost, easily reversible actions are the current sweet spot for agentic automation; the other three options are high-cost, hard-to-reverse, strategic decisions that still need a human fully in the loop.
...
**B** — safety stock is designed to absorb exactly this kind of gap for a while; the visible impact begins once that buffer runs out, not the moment the upstream event occurs.
...
**B** — a SPOF's damage is scoped to whatever depends on it; Memphis and Reno's product flow didn't depend on Andes Stitch Works, so they were structurally insulated from this particular disruption.
...
**B** — a negative net benefit under stated assumptions is a real, useful finding, not an error — it means the dollar case alone doesn't clear the bar, and a full recommendation has to weigh what the model didn't capture.
