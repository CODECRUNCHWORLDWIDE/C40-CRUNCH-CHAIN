...
**B** — Receiving → Putaway → Storage → Picking → Packing → Shipping. Product enters once, is stored, then is picked, packed, and shipped out.
...
**B** — roughly half of warehouse labor hours, because picking scales with **order-line count**, which is almost always far higher than shipment count (Lecture 1's Austin East example: 225 pick lines from a handful of inbound receipts).
...
**C** — travel time, 50–70% of a pick, reduced by better slotting (put things closer, Lecture 2) and by batching (fewer trips for the same work, Lecture 3).
...
**B** — the slots closest to the pack/ship station; the highest-frequency SKUs belong there because every trip to that slot is multiplied by how often it's picked.
...
**B** — A ≤ 80% cumulative share, B = 80–95%, C = the remaining tail. The *SKU-count* split (like "20% of SKUs") is a common outcome, not the rule itself — the rule is the cumulative percentage.
...
**B** — the SKU with 20 picks. Cumulative: 30→(30/65=46.2%), 30+20=50→(76.9%), 30+20+10=60→(92.3%). The 20-pick SKU's cumulative (76.9%) is still ≤80%; the next SKU (10 picks) pushes it to 92.3%, over the cutoff, so it lands in Class B.
...
**B** — a running (cumulative) total: for each row, it sums every row from the top of the ranking down through the current row, which is exactly what a cumulative-share calculation needs.
...
**C** — it converts a one-way slot distance into a round trip. A pick means walking to the slot *and back*; omitting the `×2` understates every travel calculation in this week by exactly half.
...
**C** — sort SKUs by pick frequency descending, sort slots by distance ascending, pair them position-for-position. This is the assignment-problem solution for minimizing total (frequency × distance) cost.
...
**B** — discrete picking makes one trip to a slot for every order line that needs it; batch picking consolidates all orders in a wave so each distinct slot is visited once per wave no matter how many orders in that wave need it.
...
**B** — bigger waves generally improve travel efficiency (more overlap, fewer trips) but can hurt cycle time, because an order has to wait for its wave to be released, and a bigger wave takes longer to fill. This is a real trade-off, not a free win in either direction.
...
**B** — 2 pickers. Pick rate 150 lines/hr = 0.4 min/line. Required minutes = 950 × 0.4 = 380. Usable minutes/picker at 80% of 450 productive minutes = 360. Pickers = ceil(380/360) = ceil(1.056) = **2** — even a small overage rounds up to a full extra picker, because a fraction of a picker isn't schedulable.
...
**B** — 64 minutes (9 + 4 + 6 + 45), the full release-to-ship elapsed time. Pick time alone (9 minutes) badly understates the real customer-facing cycle time — in this example, dock-wait time is by far the largest component, a very common real-world pattern.
